<template>
    <a-upload v-bind='mergedAttrs' :customRequest='handleCustomRequest' @change='handleChange'>
        <!-- 暴露插槽用于自定义渲染 -->
        ...
        <template v-if='showProgress' #progress='{ file }'>
            <div class='custom-progress'>
                <a-progress :percent='uploadProgress[file.uid]' :strokeWidth='progressStrokeWidth' :strokeColor='progressColor' :showInfo='false' />
...
<script setup>
    ...
    const emit = defineEmits(['update:modelValue', 'progress', 'upload-success', 'upload-error', 'change'])
    ...
    // 处理上传状态变化
    const handleChange = ({ file, fileList: updatedFiles }) => {
